...
fs.readdirSync('./CONFIGS/SERVERS/').forEach(file => {
    const serverContents = require(`../../CONFIGS/SERVERS/${file}`);
            ...
            if(messageIdentifier > 1000) {
                ...
                if(typeof messageContent != 'object') {
                    ...
                    if(messageIdentifier == 0) {
                        ...
                        } else if(messageContent.includes("[PrivateMessages]") && server.SERVER_LOGGING.PRIVATE_MESSAGES.ENABLED) {
        
                            let hook = new discord.WebhookClient({ url: server.SERVER_LOGGING.PRIVATE_MESSAGES.LOG_WEBHOOK });
                            let privateMessageRegex = /\[PM\](\w+)->(\w+):(.+)/;
                            let matches = messageContent.split("[PrivateMessages] ")[1].match(privateMessageRegex);
        
                            if(!matches) return;
                            let message = `🤫 **${matches[1]}** messaged **${matches[2]}**: ${matches[3]}`;
                        ...
                        } else if(messageContent.toLowerCase().includes("was killed by") && !messageContent.toLowerCase().includes("was killed by suicide") && !server.SERVER_LOGGING.KILL_LOGS.USE_PLUGIN_KILL_LOGS) {
                            let steamIds = messageContent.match(re);
...
async function handleF7Report(f7Object, server) {
    ...
    let subject = f7Object.Subject.split(" ")[1];
    let simpleMessage = `🚨 **[${f7Object.PlayerName}](<${reporterPlayer.profile_url}>)** has reported **[${f7Object.TargetName}](<${reportedPlayer.profile_url}>)**`+" ``"+`(${reportedPlayer.report_count+1} total reports)`+"`` for ``"+ `${subject}: ${f7Object.Message}` +"``";
    let fancyMessage = `**Reporter:** [${f7Object.PlayerName}](<${reporterPlayer.profile_url}>)\n**Reported:** [${f7Object.TargetName}](<${reportedPlayer.profile_url}>)\n**Reason**: ${f7Object.Type}\n\n**${subject}**`+"```"+f7Object.Message+"```";
...
function addPlayerData(steamId, server) {
    return new Promise((resolve, reject) => {
        db.get("select * from player_info where steam_id = ? and server_id = ?;", [ steamId, server.SERVER_SPECIAL_ID ], async function(err, row) {
            ...
            if(!row) {
                ...
                if(steamInfo) db.run("insert into player_info (steam_id, server_id, picture, name, profile_url, lastUpdated) values (?,?,?,?,?,?)", [ steamId, server.SERVER_SPECIAL_ID, steamInfo.avatarfull, steamInfo.personaname, steamInfo.profileurl, Date.now() / 1000 ]);
                db.get("select * from player_info where steam_id = ? and server_id = ?;", [ steamId, server.SERVER_SPECIAL_ID ], async function(err, addedUser) { playerData = addedUser });
            } else if(playerData?.lastUpdated + (config.PLAYER_PROFILER['Update player steam info every x hours (Will update a users steam data once a user joins the server after this threshold)'] * 3600) <= Date.now() / 1000) {
                let steamInfo = await getSteamData(steamId);
...
async function handleBanCheck(playerInfo, server) {
    if(playerInfo) {
        if(server.PLAYER_ACCOUNT_CHECKS.BAN_CHECKER.ENABLED && !playerInfo.checker_whitelisted) {
            ...
            console.log(bansArray.length);
            console.log(steamBans.NumberOfVACBans);
            console.log(steamBans.NumberOfGameBans);
            console.log(steamBans.DaysSinceLastBan);
...
function leaderboard(server, client) { 
    return new Promise((resolve, reject) => {
        ...
        if(server.LEADERBOARD.ENABLED) {
            ...
            setInterval(() => {
                db.serialize(() => {
                    db.get("select * from leaderboard where channel_id = ? and server_id = ?;", [ server.LEADERBOARD.CHANNEL_ID, server.SERVER_SPECIAL_ID ], async function(err, row) {
                        if(err) console.log(err);
                        if(row) {
                            channel.messages.fetch(row.message_id).then(messageId => {
                                const embed = new discord.EmbedBuilder();
                                ...
                                db.all(leaderboardCall1, [ server.SERVER_SPECIAL_ID ], async function(err, table) {
                                    if(err) console.log("Got error from leaderboard call 1\n" + err);
                                    if(table != undefined && table.length > 0) {
                                        let serverLeaderboardInfo;
                                        ...
                                        db.all(leaderboardCall2, [server.SERVER_SPECIAL_ID], async function(err, allInfo) {
                                            ...
                                            const attachment = new discord.AttachmentBuilder(`./src/images/imagestorage/leaderboard_${server.SERVER_SPECIAL_ID}.png`);
                                            embed.setImage(`attachment://leaderboard_${server.SERVER_SPECIAL_ID}.png`)
                                            messageId.edit({ components: [buttonList], embeds: [embed.setTitle(`${server.SERVER_SHORTNAME} ${(server.LEADERBOARD["DEFAULT_DISPLAY(wipe, lifetime)"] = "wipe" ? "WIPE STATS" : "LIFETIME STATS")}`).setImage(`attachment://leaderboard_${server.SERVER_SPECIAL_ID}.png`).setTimestamp().setFooter({text:"Leaderboard / last updated"})], files: [attachment] });  
                                        });
                                    }
                                });
                            }).catch(err => {
