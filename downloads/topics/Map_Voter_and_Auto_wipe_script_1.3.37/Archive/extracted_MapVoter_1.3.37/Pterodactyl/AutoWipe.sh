...
while true ; do
  ...
  then
	  ...
	  then
		...
		if [ ! -e ${arr[0]} ] 
		then
			echo "${arr[0]} not found.if it exist try removing ./ at the start of the path in AutoWipeConfig.txt" >> $PWD/AutoWipe.log
			./discord.sh --webhook-url=${arr[2]} --username "Auto wipe" --avatar "https://cdn-icons-png.flaticon.com/512/1999/1999208.png" --text "${arr[0]} not found.if it exist try removing ./ at the start of the path in AutoWipeConfig.txt"
		fi 
		if [ ! -e ${arr[1]} ]
		then
			echo "${arr[1]} not found.if it exist try removing ./ at the start of the path in AutoWipeConfig.txt" >> $PWD/AutoWipe.log
			./discord.sh --webhook-url=${arr[2]} --username "Auto wipe" --avatar "https://cdn-icons-png.flaticon.com/512/1999/1999208.png" --text "${arr[1]} not found.if it exist try removing ./ at the start of the path in AutoWipeConfig.txt"
		fi
		if [ -e ${arr[0]} ] && [ -e ${arr[1]} ]
  		then
			...
			then
				...
				then
					...
					./discord.sh --webhook-url=${arr[2]} --username "Auto wipe" --avatar "https://cdn-icons-png.flaticon.com/512/1999/1999208.png" --text "Deleting all maps and blueprints files"
				fi
				if [ "$ForcedWipe" == "true" ]; 
	  			then
					echo $(date '+%d/%m/%Y %H:%M:%S') "Updating the server"   >> $PWD/AutoWipe.log
  ...
  ./RustDedicated -batchmode $1 $2 $3 $4 $5 $6
  done
